...
public class PortableStockTickerScreen extends AbstractSimiContainerScreen<PortableStockTickerMenu>
    ...
    protected void renderBg(@NotNull GuiGraphics pGuiGraphics, float partialTicks, int mouseX, int mouseY) {
        ...
        pGuiGraphics.drawString(font, headerTitle, x + windowWidth / 2 - font.width(headerTitle) / 2, y + 4, 0x714A40,
                false);
        MutableComponent component = CreateLang.translate("gui.stock_keeper.send")
                .component();
        ...
        // BG
        for (int sliceY = -2; sliceY < getMaxScroll() * rowHeight + windowHeight
                - 72; sliceY += AllGuiTextures.STOCK_KEEPER_REQUEST_BG.getHeight()) {
    ...
    private void renderItemEntry(GuiGraphics graphics, float scale, BigGenericStack entry, boolean isStackHovered,
        ...
        if (customCount != 0 || craftable) {
            boolean renderedCustomAmount = false;
            if (Mods.FLUIDLOGISTICS.isLoaded() && CFLBridge.isVirtualFluid(entry)) {
                ms.pushPose();
                ms.translate(1, 1, 0);
                renderedCustomAmount = CFLClientBridge.renderStockKeeperAmount(
                        graphics, CFLBridge.keyAsItemStack(entry.get()), customCount);
                ms.popPose();
            }
            if (!renderedCustomAmount) {
                GenericContentExtender.registrationOf(entry.get().key())
                        .clientProvider().guiHandler()
    ...
    protected void renderForeground(@NotNull GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if (hoveredSlot != noneHovered) {
            ...
            ArrayList<Component> lines = Mods.FLUIDLOGISTICS.isLoaded() && CFLBridge.isVirtualFluid(entry)
                    ? CFLBridge.getStockKeeperTooltip(entry, recipeHovered, orderHovered,
        ...
        if (ClientScreenStorage.stacks != null) {
            ClientScreenStorage.stacks.forEach(stack -> {
                if (stack.amount() > MAX_REPORTED_STACK_AMOUNT) {
                    if (Mods.FLUIDLOGISTICS.isLoaded() && CFLBridge.isVirtualFluid(stack)) {
    ...
    public boolean mouseClicked(double pMouseX, double pMouseY, int pButton) {
        ...
        int transfer;
        boolean isCFLFluidEntry = Mods.FLUIDLOGISTICS.isLoaded() && CFLBridge.isVirtualFluid(entry);
        boolean isCFLFluid = isCFLFluidEntry && !recipeClicked;
        if (recipeClicked && isCFLFluidEntry) {
            transfer = CFLBridge.getFluidRecipeStepAmount(entry.get(), hasShiftDown(), hasControlDown());
        } else if (isCFLFluid) {
        ...
        if (isCFLFluid) {
            int newAmount = CFLBridge.adjustFluidRequestAmount(entry.get(), current, !(rmb || orderClicked),
                    hasShiftDown(), hasControlDown(), 0, Math.max(0, entry.get().amount()), 1, orderClicked);
            if (newAmount <= 0) {
    ...
    public void requestCraftable(CraftableGenericStack cbis, int requestedDifference) {
        orderForStackCallCount.set(0);
        if (Mods.FLUIDLOGISTICS.isLoaded() && CFLBridge.hasCustomRecipeData(cbis.asStack())) {
            requestCustomFluidCraftable(cbis, requestedDifference);
            return;
        }
    ...
    private void requestCustomFluidCraftable(CraftableGenericStack cbis, int requestedDifference) {
        CraftableBigItemStack craftStack = cbis.asStack();
        int outputCount = CFLBridge.getCustomOutputCount(craftStack);
        List<BigItemStack> requirements = CFLBridge.getCustomRequirements(craftStack);
        if (outputCount <= 0)
            return;
    ...
    private int getCustomOrderCount(ItemStack target) {
        ...
        for (BigGenericStack order : itemsToOrder) {
            if (ItemStack.isSameItemSameComponents(order.asStack().stack, target))
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        ...
        try {
            ...
            boolean isCFLFluidEntry = Mods.FLUIDLOGISTICS.isLoaded() && CFLBridge.isVirtualFluid(entry);
            boolean isCFLFluidScroll = isCFLFluidEntry && !recipeClicked;
            int steps = Mth.ceil(Math.abs(scrollY));
            int transfer;
            if (recipeClicked && isCFLFluidEntry) {
                transfer = CFLBridge.getFluidRecipeStepAmount(entry.get(), hasShiftDown(), hasControlDown()) * steps;
            } else if (isCFLFluidScroll) {
            ...
            if (isCFLFluidScroll) {
                ...
                int newAmount = CFLBridge.adjustFluidRequestAmount(entry.get(), current, !remove, hasShiftDown(),
                        hasControlDown(), 0, Math.max(0, summary.getCountOf(entry.get().key())), steps, orderClicked);
                if (newAmount <= 0) {
                    itemsToOrder.remove(existingOrder);
                    playUiSound(SoundEvents.WOOL_STEP, 0.75f, 1.8f);
                    playUiSound(SoundEvents.BAMBOO_WOOD_STEP, 0.75f, 1.8f);
        ...
        if (!itemsToOrder.isEmpty() && !recipesToOrder.isEmpty()) {
            ...
            for (CraftableGenericStack cbis : recipesToOrder) {
                ...
                PackageOrder pattern = new PackageOrder(
                        FactoryPanelScreen.convertRecipeToPackageOrderContext(cr, stacks, true));
